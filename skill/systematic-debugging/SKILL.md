...
6. **记录假设-验证矩阵**
   ...
   在提出修复方案之前，把你所有的假设列出来：

   ```
   | 假设 | 验证方法 | 结果 |
   |------|---------|------|
   | X 导致了 Y | grep / 断点 / 最小复现 | 成立 / 不成立 |
   ```

   **规则：**
   - 未经验证的假设不允许进入第四阶段（实施）
   - "我觉得是 X"不是假设，"X 导致 Y，因为 Z 证据"才是
   - 如果一个假设无法被验证，说明你缺少可观测性——回到第 4 步

### 跨语言/跨进程系统的额外约束

当系统涉及多个语言运行时（如 Rust ↔ Python、Node ↔ Go）或跨进程通信时，
除上述流程外，**必须额外满足以下约束**：

#### 可观测性前置（不可跳过）

在开始调试之前，先确认可观测性基础设施就位：

- [ ] 子进程 stderr 被捕获并转发到主进程日志
- [ ] 每个组件边界有带时间戳的请求/响应日志
- [ ] 存在健康检查端点（如 HTTP /health）
- [ ] 日志级别支持运行时切换

**如果以上任一未满足，先补基础设施再调试。**
不要边修 bug 边加日志——这是 55% 调试时间的浪费来源。

#### 参数契约检查

- [ ] 跨语言共享参数（采样率、chunk 大小、协议版本）是否有单一来源？
- [ ] 参数变更是否在两端同步？
- [ ] 最近一次提交是否意外修改了共享参数？

**混提交是参数被无意破坏的主要来源。**
检查最近提交的 diff 范围是否超出了预期修改的文件。
...
2. 接收方是否收到了请求？（检查接收日志）
3. 处理方是否产生了结果？（检查处理日志）
4. 结果是否返回到了调用方？（检查返回日志）
5. 调用方是否正确处理了结果？（检查消费逻辑）
```

找到第一个"输入正常但输出异常"的节点，那就是根因。
